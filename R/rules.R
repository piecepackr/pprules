...
#'                  `list.files(system.file("rules", package = "pprules"), full.names = TRUE)`.
...
save_ruleset <- function(
	...
	knit_game(game, gk, quietly, size, game_files = game_files, output_ext = output_ext, wd = dir)

	of <- system.file("templates/ruleset.Rtex", package = "pprules")
...
save_pamphlet <- function(
	...
	of <- system.file("templates/pamphlet.Rtex", package = "pprules")
	tex <- knit(of, quiet = quietly)
	if (output_ext == "pdf") {
		pdf <- xelatex(tex, quietly)
		embed_xmp(pdf, game, game_info)
...
save_pocketmod <- function(
	...
	of <- system.file("templates/pocketmod1.Rtex", package = "pprules")
	tex <- knit(of, quiet = quietly)
	pdf <- xelatex(tex, quietly)

	of <- system.file("templates/pocketmod2.Rtex", package = "pprules")
	tex <- knit(of, quiet = quietly)
	pdf <- xelatex(tex, quietly)

	of <- system.file("templates/pocketmod3.Rtex", package = "pprules")
	tex <- knit(of, quiet = quietly)
	pdf <- xelatex(tex, quietly)
...
knit_chapter <- function(
	...
	of <- system.file("templates/chapter.Rtex", package = "pprules")
	tex <- knit(of, quiet = quietly)
...
save_rulebook <- function(
	...
	## games <- list.files(system.file("rules", package = "pprules"), pattern = ".Rtex")
	## games <- gsub(".Rtex", "", games)
	games <- names_rulesets(book = book) %>% normalize_name(sep = "-")
	for (game_hyphen in games) {
		knit_chapter(game_hyphen, gk, quietly, size, game_info = game_info, output_ext = output_ext)
	}

	## new argument book files?
	of <- system.file(str_glue("books/{book_hyphen}.Rtex"), package = "pprules")
	tex <- knit(of, quiet = quietly)
	if (output_ext == "pdf") {
		pdf <- xelatex(tex, quietly)
		file.copy(pdf, output, overwrite = TRUE)
...
game_credits <- function(game, game_info = NULL) {
	info <- get_game_info(game, game_info)
	items <- list()
	if ("author" %in% names(info)) {
		items$`Written by:` <- info$author
	}
	...
	license <- paste(copyright, paste("License: ", license_text, "\\newline"), collapse = "\n")
	cat(paste(
		c(
			"\\begin{description}",
			str_glue("\\item[{names(items)}] {as.character(items)}"),
...
pdf_metadata <- function(
	...
	if (is.null(pdftitle)) {
		pdftitle <- title(game, game_info)
	}
	if (is.null(pdfauthor)) {
		pdfauthor <- author(game, game_info)
	}
	if (is.null(pdfsubject)) {
		pdfsubject <- subject(game, game_info)
	}
	if (is.null(pdfkeywords)) {
		pdfkeywords <- keywords(game, game_info)
	}
	cat(
		paste0("\\hypersetup{", pdf_key("pdftitle", pdftitle)),
		pdf_key("pdfauthor", pdfauthor),
		pdf_key("pdfcreator", paste0("pprules (v", packageVersion("pprules"), ")")),
