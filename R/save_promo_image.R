...
save_promo_image <- function(game, gk = game_kit(), file = NULL, ...) {
	...
	if (is.null(file)) {
		file <- paste0(game_name, "_promo.pdf")
	}
	...
	switch(
		...
		tablut = {
			df <- piecepack_tablut(cfg$get_width("die_face"))
			wh <- render_piece(
				df,
				file = file,
				...,
		...
		{
			df <- piecepack_by_name(game_name, getter = pprules_get)
			wh <- render_piece(
				df,
				file = file,
				...,
...
promo_morris_df <- function() {
	df_tiles <- piecepack_nine_mens_morris()
	df_coins <- tibble(
...
promo_pass_the_food_df <- function() {
	df_tiles <- piecepack_pass_the_food()
	withr::local_seed(36)
	df_coins <- tibble(
		piece_side = "coin_back",
		rank = rep(1:6, 4),
...
promo_speedy_towers_df <- function() {
	withr::local_seed(72)
	df_tiles <- tibble(piece_side = "tile_face", suit = rep(1:4, 6), rank = rep(1:6, each = 4))[
		sample.int(24L),
	]
	...
	df_coins <- tibble(piece_side = "coin_back", suit = rep(1:4, 6), rank = rep(1:6, each = 4))[
		sample.int(24L),
	]
